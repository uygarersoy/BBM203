...
public:
    ...
    vector<vector<vector<bool>>> createBlocksFromFile(const string& input_file);
    vector<vector<int>> transpose(vector<vector<int>> matrix);
    vector<vector<int>> reverseRows(vector<vector<int>> matrix);
    vector<vector<bool> > rotateMatrix90Degrees(vector<vector<bool>> matrix);
    vector<vector<int>> convertVectorToInt(vector<vector<bool>> inputVec);
    vector<vector<bool>> convertVectorToBool(vector<vector<int>> inputVec);
    Block * linkBlocks(const vector<Block *> &blocksVector);
    vector<Block *> rotations(Block * block);
    void print(vector<vector<bool>> in);
};
